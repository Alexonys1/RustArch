...
#[test]
fn huffman_single_symbol_only_compresses_when_header_is_smaller() {
    for size in [1, 11] {
        let source = vec![0x5A; size];
        let (effective, stored_size, _) =
            compression_round_trip(CompressionId::Huffman, &source);

        assert_eq!(effective.compression, CompressionId::NoCompression);
        assert_eq!(stored_size, size as u64);
    }

    let source = vec![0x5A; 12];
    let (effective, stored_size, _) =
        compression_round_trip(CompressionId::Huffman, &source);
